grapesjs.plugins.add('grapesjs-blocks-table', (editor, options) => {
  ...
  editor.DomComponents.addType('customTable', {
  ...
  const getCellToolbar = () => {
    let toolbar = [
      { attributes: { class: "fa fa-columns columns-operations", title: 'Columns operations' }, command: "table-show-columns-operations" },
      { attributes: { class: "fa fa-columns rows-operations", title: 'Rows operations' }, command: "table-show-rows-operations" },
  ...
  editor.Commands.add('table-show-columns-operations', editor => {
    updateTableToolbarSubmenu('columns', 'rows');
  });

  editor.Commands.add('table-show-rows-operations', editor => {
    updateTableToolbarSubmenu('rows', 'columns');
  });

  editor.Commands.add('table-select', editor => {
  ...
  editor.Commands.add('table-insert-row-below', editor => {
    let selected = editor.getSelected();
    if (selected.is('cell')) {
      let rowComponent = selected.parent();
      rowComponent.parent().components().add({
  ...
  editor.Commands.add('table-insert-column-left', editor => {
    let selected = editor.getSelected();
    if (selected.is('cell')) {
  ...
  editor.Commands.add('table-insert-column-right', editor => {
    let selected = editor.getSelected();
    if (selected.is('cell')) {
      let columnIndex = selected.collection.indexOf(selected);
      selected.parent().parent().components().forEach(component => {
  ...
  editor.Commands.add('table-delete-row', editor => {
    let selected = editor.getSelected();
    if (selected.is('cell')) {
  ...
  editor.Commands.add('table-delete-column', editor => {
    let selected = editor.getSelected();
    if (selected.is('cell')) {
      let table = selected.parent().parent();
      let columnIndex = selected.collection.indexOf(selected);
  ...
  editor.Commands.add('table-merge-cells-right', editor => {
    let selected = editor.getSelected();
    if (selected.is('cell')) {
      ...
      if (columnIndex + 1 == selected.parent().components().length) {
        jQuery('#button-merge-cells-right').hide();
      }
    }
  });

  editor.Commands.add('table-merge-cells-down', editor => {
    let selected = editor.getSelected();
    if (selected.is('cell')) {
      let currentColspan = selected.getAttributes()['colspan'] ? selected.getAttributes()['colspan'] : 1;
      ...
      if (rowIndex + 2 == table.components().length) {
        jQuery('#button-merge-cells-down').hide();
      }
    }
  });

  editor.Commands.add('table-unmerge-cells', editor => {
    let selected = editor.getSelected();
    if (selected.is('cell')) {
      let currentColspan = selected.getAttributes()['colspan'] ? selected.getAttributes()['colspan'] : 1;
      let currentRowspan = selected.getAttributes()['rowspan'] ? selected.getAttributes()['rowspan'] : 1;
      ...
      editor.selectRemove(selected);
      setTimeout(function() { editor.select(selected); }, 50);
      jQuery('#button-merge-cells-down').show();
      jQuery('#button-merge-cells-right').show();
    }
  });
...
function updateTableToolbarSubmenu (submenuToShow, submenuToHide) {
  ...
  if (selected && selected.is('cell')) {
    if (jQuery('.' + submenuToHide + '-operations .toolbar-submenu').length > 0){
      jQuery('.' + submenuToHide + '-operations .toolbar-submenu').slideUp('slow');
    }
    if (jQuery('.' + submenuToShow + '-operations .toolbar-submenu').length > 0){
      if (jQuery('.' + submenuToShow + '-operations .toolbar-submenu').css('display') != 'none') {
        jQuery('.' + submenuToShow + '-operations .toolbar-submenu').slideUp('slow');
        return;
      }
      jQuery('.' + submenuToShow + '-operations .toolbar-submenu').slideDown('slow');
    } else {
      let htmlString = '';
      ...
      jQuery('.' + submenuToShow + '-operations').append(htmlString);
      jQuery('.' + submenuToShow + '-operations .toolbar-submenu').slideDown('slow');
    }
  }
}
